...
./Tools/autotest/param_metadata/param_parse.py --vehicle ArduCopter --format xml_mp || true
...
./Tools/autotest/param_metadata/param_parse.py --vehicle ArduPlane --format xml_mp || true
...
./Tools/autotest/param_metadata/param_parse.py --vehicle APMrover2 --format xml_mp || true
echo "Remove the two first lines and the last one"
sed -i -e '1d' -e '2d' -e '$d' ParameterMetaData.xml
echo "Append parameters to the complete file"
cat ParameterMetaData.xml >> ParameterMetaDataBackup.xml

./Tools/autotest/param_metadata/param_parse.py --vehicle ArduSub --format xml_mp || true
echo "Remove the two first lines and the last one"
sed -i -e '1d' -e '2d' -e '$d' ParameterMetaData.xml
echo "Append parameters to the complete file"
cat ParameterMetaData.xml >> ParameterMetaDataBackup.xml

./Tools/autotest/param_metadata/param_parse.py --vehicle AntennaTracker --format xml_mp || true
